...
public abstract class AbstractHonoClient {
    ...
    /**
     * Marks an <em>OpenTracing</em> span as erroneous and logs an event.
     * <p>
    ...
    protected final void logError(final Span span, final Throwable error) {
        if (span != null) {
            ...
            events.put("description", error.getMessage());
            logError(span, events);
        }
    }
     ...
     * 
     * @param span The span to mark.
     * @param event The event to log on the span.
     */
    protected final void logError(final Span span, final String event) {
        if (span != null) {
    ...
    /**
     * Marks an <em>OpenTracing</em> span as erroneous and logs several events.
     * <p>
     * This method does <em>not</em> finish the span.
     * 
     * @param span The span to finish.
     * @param events The events to log on the span.
     */
    protected final void logError(final Span span, final Map<String, ?> events) {
        if (span != null) {
            Tags.ERROR.set(span, Boolean.TRUE);
            span.log(events);
        }
     ...
     * <ul>
     * <li>{@link Tags#COMPONENT} - set to {@link ClientConfigProperties#getName()}</li>
    ...
    private Span newSpan(final SpanContext parent, final String referenceType, final String operationName) {
        ...
        return tracer.buildSpan(operationName)
                    ...
                    .withTag(Tags.COMPONENT.getKey(), config.getName() == null ? "hono-client" : config.getName())
