...
/**
 * A consumer that consumes <em>event</em> messages from Kafka.
 * <p>
 * The event consumer needs to be closed by invoking {@link #close()} ()} when it is no longer needed.
 */
public class EventConsumer extends DownstreamMessageConsumer {

    private EventConsumer(final KafkaConsumer<String, Buffer> kafkaConsumer, final String topic,
    ...
    /**
     * Creates and starts a message consumer for events.
     *
    ...
    public static Future<MessageConsumer> create(final KafkaConsumer<String, Buffer> kafkaConsumer,
        ...
        final EventConsumer consumer = new EventConsumer(kafkaConsumer, topic, messageHandler, closeHandler,
                pollTimeout);
